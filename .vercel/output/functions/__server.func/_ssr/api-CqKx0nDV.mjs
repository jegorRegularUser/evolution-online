...
import { S as pollInput, _ as joinRoomInput, a as botsInput, b as legalFeedActions, c as chooseAIAction, d as createRoomInput, f as currentActor, i as applyAction, l as codeTokenInput, r as actionInput, t as PACE, u as createGame, v as legalDefenseActions, y as legalDevActions } from "./ai-CpbfIBmp.mjs";
//#region node_modules/.nitro/vite/services/ssr/assets/api-CqKx0nDV.js
...
async function readRoom(sql, code) {
	const r = (await sql.query(`select code, status, capacity, difficulty, version, state as state_json,
	...
	return {
		async create(input) {
			...
			for (let attempt = 0; attempt < 6; attempt++) {
				const code = makeCode();
				try {
					await sql.query(`insert into evo_rooms (code, capacity, difficulty, seed)
             values ($1, $2, $3, $4)`, [
						code,
						input.capacity,
						input.difficulty,
		...
		async start(code, token) {
			...
			const state = createGame(room.capacity, room.difficulty, Math.floor(Math.random() * 1e6), defs);
...
function getRoomService() {
	const g = globalThis;
	g.__evoNetService__ ??= import("./db-B_nKkhdt.mjs").then(({ getSql }) => getSql().then((sql) => createRoomService(sql)));
