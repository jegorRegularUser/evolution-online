...
/**
 * Windows `spawn()` без shell не запускает `.cmd`-шимы из `node_modules/.bin`
 * (spawn("vite") → ENOENT), а shell ломает пересылку сигналов. Известный
 * JS-вход Vite запускаем текущим node напрямую; остальное — через shell.
 */
export function resolveSpawnPlan(command, args, root, platform = process.platform) {
...
function main(argv) {
  ...
  const plan = resolveSpawnPlan(command, args, root);
