 ...
 *   - Sandbox live preview: the shared preview client is usable only when the
 *     broker secret is supplied in `GROK_PREVIEW_CLIENT_SECRET`; no secret is
 *     embedded in this repository. The preview's dynamic
 *     `https://*.grok-sandbox.com` origin is derived from the request.
...
/** Читает переменную окружения, считая пустую строку unset. */
const env = (key: string): string | undefined => {
  const value = process.env[key]?.trim();
  return value ? value : undefined;
};

// Явный выключатель. Владелец проекта отключил аккаунты в `.grok/app-env.json`;
// при отсутствии секретов и включённом флаге ниже будет fail-fast с понятной
// ошибкой, а не тихим использованием устаревшего credential из исходников.
const authDisabled = env("VITE_AUTH_ENABLED") === "false";

// OAuth-конфигурация читается только из окружения. Preview-клиент не имеет
// credential в репозитории: его secret обязан прийти из secret store.
const grokIssuer = env("GROK_AUTH_ISSUER") ?? GROK_ISSUER_DEFAULT;
const grokClientId = env("GROK_AUTH_CLIENT_ID") ?? readPreviewClientId();
const grokClientSecret =
  env("GROK_AUTH_CLIENT_SECRET") ?? readPreviewClientSecret();
const betterAuthSecret = env("BETTER_AUTH_SECRET");

/** True when federated sign-in is active (real auth is enforced). */
export const authConfigured = !authDisabled &&
  Boolean(grokClientId && grokClientSecret && betterAuthSecret);

if (!authDisabled && !authConfigured) {
  const missing = [
    !grokClientId && "GROK_AUTH_CLIENT_ID (или GROK_PREVIEW_CLIENT_ID)",
    !grokClientSecret && "GROK_AUTH_CLIENT_SECRET (или GROK_PREVIEW_CLIENT_SECRET)",
    !betterAuthSecret && "BETTER_AUTH_SECRET",
  ].filter(Boolean);
  throw new Error(
    `[auth] Авторизация включена, но не заданы переменные окружения: ${missing.join(", ")}. ` +
      "Задайте их в secret store или установите VITE_AUTH_ENABLED=false.",
  );
}
...
export const auth = betterAuth({
  baseURL,
  // Better Auth signing key тоже приходит только из окрущения. Инертный
  // sentinel используется лишь при явном VITE_AUTH_ENABLED=false, когда auth
  // выключен и cookie/session API не вызываются; это не credential и не
  // замена секрету для включённого auth.
  secret: betterAuthSecret ?? env("VITE_AUTH_ENABLED"),
  database,
